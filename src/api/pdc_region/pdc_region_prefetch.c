...
pdc_region_prepare_global_prefetch_list()
{
    ...
    *local_bytes     = total_bytes;
    ...
    global_offset_list =
        (uint64_t)PDC_malloc(obj_prefetch_list_len * pdc_client_mpi_size_g * sizeof(uint64_t));
    global_size_list = (uint64_t)PDC_malloc(obj_prefetch_list_len * pdc_client_mpi_size_g * sizeof(uint64_t));

    MPI_Allgather(reg_offset_list, obj_prefetch_list_len, MPI_UINT64_T, global_offset_list,
                  obj_prefetch_list_len, MPI_UINT64_T, MPI_COMM_WORLD);
    MPI_Allgather(reg_size_list, obj_prefetch_list_len, MPI_UINT64_T, global_size_list, obj_prefetch_list_len,
