                                    ...
                                    int *global_list_len)
{
            ...
            for (j = 0; j < global_list_len[i]; j++) {
                ...
                if (strcmp(obj_cache_iter->obj_name, global_prefetch_list[prefetch_list_idx]) == 0) {
                    ...
                    if (!region_compare) {
                        ...
                        is_contained = detect_region_contained(&offset[i], &size[i], obj_cache_iter->reg_offset,
    ...
    obj_cache_iter = obj_cache_list;
    while (obj_cache_iter != NULL) {
        printf("rank: %d, obj_name: %s, target_rank: %d\n", pdc_client_mpi_rank_g, obj_cache_iter->obj_name, obj_cache_iter->target_rank);
        obj_cache_iter = obj_cache_iter->next;
    }

    pdc_region_cache_timelog(start,
                             "pdc_region_dl_prepare_data_exchange - total time");
...
pdc_region_dl_data_pack(char *send_buf, struct pdc_object_cache *obj_cache_item, int total_size) {
    ...
    MPI_Pack(obj_cache_item->obj_name, MAX_NAME_LEN, MPI_CHAR, send_buf, total_size, &position,
             client_cache_comm);
    MPI_Pack(&(obj_cache_item->unit), 1, MPI_UINT64_T, send_buf, total_size, &position,
             client_cache_comm);

    MPI_Pack(&(obj_cache_item->reg_ndim), 1, MPI_INT, send_buf, total_size, &position,
             client_cache_comm);
    MPI_Pack(obj_cache_item->reg_offset, 3, MPI_UINT64_T, send_buf, total_size, &position,
             client_cache_comm);
    MPI_Pack(obj_cache_item->reg_size, 3, MPI_UINT64_T, send_buf, total_size, &position,
             client_cache_comm);

    MPI_Pack(&(obj_cache_item->buf_size), 1, MPI_UINT64_T, send_buf, total_size, &position,
             client_cache_comm);
    MPI_Pack(obj_cache_item->buf, obj_cache_item->buf_size, MPI_CHAR, send_buf, total_size, &position, client_cache_comm);
...
pdc_region_dl_data_unpack(char *recv_buf, struct pdc_object_cache *obj_cache_item, int total_size) {
    ...
    MPI_Unpack(recv_buf, total_size, &position, obj_cache_item->obj_name, MAX_NAME_LEN, MPI_CHAR, client_cache_comm);
    ...
    MPI_Unpack(recv_buf, total_size, &position, obj_cache_item->reg_offset, 3, MPI_UINT64_T, client_cache_comm);
    MPI_Unpack(recv_buf, total_size, &position, obj_cache_item->reg_size, 3, MPI_UINT64_T, client_cache_comm);

    MPI_Unpack(recv_buf, total_size, &position, &(obj_cache_item->buf_size), 1, MPI_UINT64_T, client_cache_comm);

    obj_cache_item->buf = (char *)PDC_malloc(sizeof(char) * obj_cache_item->buf_size);
    MPI_Unpack(recv_buf, total_size, &position, obj_cache_item->buf, obj_cache_item->buf_size, MPI_CHAR, client_cache_comm);
...
pdc_region_dl_data_exchange(int obj_prefetch_list_len) {
    ...
    // while (!local_all_done) {
        ...
        printf("[RANK %d] sends_completed_count: %d, receives_completed_count: %d, data_exchange_item_num: %d, num_segments_to_receive: %d\n", pdc_client_mpi_rank_g, sends_completed_count, receives_completed_count, data_exchange_item_num, num_segments_to_receive);
        fflush(stdout);
        ...
        if (!local_sends_done && (obj_cache_iter != NULL) && (obj_cache_iter->target_rank != -1) 
            ...
            printf("[RANK %d] obj_name: %s sent, offset: %d\n", pdc_client_mpi_rank_g, obj_cache_iter->obj_name, obj_cache_iter->reg_offset[0]);
            fflush(stdout);
            
            // MPI_Pack the node item to send
            position = pdc_region_dl_data_pack(send_buffer, obj_cache_iter, total_size);
            MPI_Isend(send_buffer, position, MPI_PACKED,
                      obj_cache_iter->target_rank, 0, client_cache_comm,
        ...
        printf("[RANK %d] send_buffer_flag: %d, obj_name: %s\n", pdc_client_mpi_rank_g, send_buffer_flag, obj_cache_iter->obj_name);
        // printf("[RANK %d] send_buffer_flag: %d, obj_name: %s, initiated: %d\n", pdc_client_mpi_rank_g, send_buffer_flag, obj_cache_iter->obj_name, obj_cache_iter->is_initiated);
        fflush(stdout);
        ...
        if (!local_receives_done && (current_recv_request != MPI_REQUEST_NULL)) {
            ...
            if (flag) { // A segment has been received
                ...
                printf("[RANK %d] Received obj_name: %s, offset: %d, head_obj_name: %s, head_obj_offset: %d\n", pdc_client_mpi_rank_g, recv_obj_item->obj_name, recv_obj_item->reg_offset[0], obj_cache_list->obj_name, obj_cache_list->reg_offset[0]);
                fflush(stdout);
