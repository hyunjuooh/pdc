...
pdc_region_dl_init()
{
    ...
    MPI_Comm_split_type(client_cache_world_comm, MPI_COMM_TYPE_SHARED, 0, MPI_INFO_NULL, &client_cache_node_comm);
...
pdc_region_dl_data_exchange(pdcid_t *global_prefetch_list, int obj_prefetch_list_len)
{
	...
	int start_idx = c * chunk_size;
        ...
        int i = 0;
        for (obj_cache_iter = client_info.local_cache_list_head; obj_cache_iter != NULL; obj_cache_iter = obj_cache_iter->next) {
	    if (i >= start_idx && i < end_idx) {
	        if (client_info.rank_to_node_id_map[obj_cache_iter->target_rank] == client_info.node_manager_rank)
		    current_intra_send++;
	        else
		    current_inter_send++;
            }
	...
	int inter_node_item_count = 0;
        ...
        MPI_Alltoall(inter_node_send_counts, 1, MPI_INT, inter_node_recv_counts, 1, MPI_INT, client_cache_world_comm);
        ...
        if (inter_node_item_count > 0) {
            ...
            for (obj_cache_iter = exchange_head; obj_cache_iter != NULL; obj_cache_iter = obj_cache_iter->next) {
                if (item_idx >= start_idx && item_idx < end_idx) {
                    int trg_rank = obj_cache_iter->target_rank;
                    if (client_info.rank_to_node_id_map[trg_rank] != client_info.node_manager_rank) {
                        long offset = (long)sdispls_inter_bytes[trg_rank] + (long)temp_offsets_inter[trg_rank] * TRANSFER_UNIT_SIZE;
