    ...
    while (obj_cache_iter != NULL) {
        if (obj_cache_iter->obj_id == obj_id) {
            ...
            while (reg_cache_iter != NULL) {
                ...
                region_contained = detect_region_contained(offset, size, reg_cache_iter->reg_offset, reg_cache_iter->reg_size, reg_cache_iter->reg_ndim);
                
                // Currently considering fully contained case
                if (region_contained) {
                    // Get the offset and size information of overlapped region part
                    PDC_region_overlap_detect(ndim, offset, size, reg_cache_iter->reg_offset, reg_cache_iter->reg_size, &overlap_offset, &overlap_size);

                    // Copy the overlapped part into the provided transfer_request buffer
                    memcpy_overlap_subregion(reg_cache_iter->reg_ndim, unit, reg_cache_iter->buf, reg_cache_iter->offset, reg_cache_iter->size, buf, offset, size, overlap_offset, overlap_size);
...
perr_t pdc_region_cache_evict(size_t required_size)
{
    ...
    while (obj_cache_iter != NULL) {
        ...
        // From the end of the region list, free the region item of the list until it matches the required size
        do {
