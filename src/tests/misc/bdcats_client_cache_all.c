...
main(int argc, char *argv[])
{
    ...
    rank_arr = (int *)malloc(size * sizeof(int));
    ...
    all_requests[0] = transfer_request_x;
    all_requests[1] = transfer_request_y;
    all_requests[2] = transfer_request_z;
    all_requests[3] = transfer_request_px;
    all_requests[4] = transfer_request_py;
    all_requests[5] = transfer_request_pz;
    all_requests[6] = transfer_request_id1;
    all_requests[7] = transfer_request_id2;
    ...
    ret = PDCregion_transfer_start_all_mpi(all_requests, 8, MPI_COMM_WORLD);
    ...
    if (rank <= 5) {
        ...
        printf("[CACHE_LOG] [%02d:%02d:%02d] [RANK %d] | Transfer Start All Time: %f\n", log_time->tm_hour,
    ...
    ret = PDCregion_transfer_wait_all(all_requests, 8);
    ...
    if (access_pattern == 1 || access_pattern == 3) {
        for (j = 0; j < mysize[0]; ++j) {
            if (id1[j] != j) {
                printf("[Rank %d] at %d id1 wrong value %d!=%d @ line %d\n", rank, j, id1[j], j, __LINE__);
                break;
            }
            if (id2[j] != j * 2) {
